...
def index(request):
    return render(request, 'task_management_system_app/index.html')

def project(request):
    return redirect(reverse('create_task'))

def teams(request):
    return render(request, 'task_management_system_app/teams.html')

def status(request):
    return redirect(reverse('task_chart'))

def employees(request):
    return render(request, 'task_management_system_app/employees.html')

def leave(request):
    # views.py

    if request.method == 'POST':
        form = Leave(request.POST)
        if form.is_valid():
            leave = form.save(commit=False)
            leave.user = request.user  # Set the user to the currently logged-in user
            leave.save()
            return redirect('leave_success')  # Redirect to a success page or another view
    else:
        form = Leave()
    
    return render(request, 'task_management_system_app/leave.html', {'form': form})